        ...
        <div className="border-b border-line/70 bg-white/70 backdrop-blur-xl backdrop-saturate-150">
          <div className="mx-auto flex h-12 max-w-page items-center justify-between px-5">
            <div className="flex items-center gap-2.5">
              ...
              <Link
                to="/"
                className="text-[17px] font-semibold tracking-[0.22em] text-ink"
              >
                SAENGDAO
              </Link>
            </div>
      ...
      <div className={`fixed inset-0 z-[60] sm:hidden ${mobileOpen ? "" : "pointer-events-none"}`}>
        ...
        <div className={`absolute left-0 top-0 flex h-full w-72 max-w-[80%] flex-col bg-white shadow-2xl transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
          ...
          <nav className="flex-1 overflow-y-auto p-3">
            {nav.map((n) => (
              <NavLink
                ...
                onClick={() => setMobileOpen(false)}
                className={({ isActive }) =>
                  `block rounded-xl px-4 py-3 text-[16px] transition ${isActive ? "bg-ink text-white" : "text-ink hover:bg-mist"}`
                }
              >
                {n.label}
              </NavLink>
          ...
          <div className="border-t border-line p-3">
            <Link to="/account" onClick={() => setMobileOpen(false)} className="block rounded-xl px-4 py-3 text-[16px] text-ink hover:bg-mist">บัญชีของฉัน</Link>
          </div>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-page px-5 py-14">
          <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="lg:col-span-1">
              <p className="text-[16px] font-semibold tracking-[0.22em]">SAENGDAO</p>
              <p className="mt-3 max-w-xs text-[14px] leading-relaxed text-sub">
            ...
            {/* ช่องทางชำระเงิน */}
            <div>
              <p className="mb-3 text-[14px] font-semibold text-ink">ชำระเงินปลอดภัย</p>
              <div className="flex flex-wrap gap-2">
                {["พร้อมเพย์", "โอนเงิน", "บัตรเครดิต"].map((p) => (
                  <span key={p} className="rounded-lg border border-line bg-white px-2.5 py-1 text-[12px] text-sub">{p}</span>
                ))}
              </div>
            </div>
          </div>

          <div className="mt-12 border-t border-line pt-6 text-[13px] text-sub">
  ...
  return (
    <div>
      ...
      <ul className="space-y-2">
        {links.map((l) => (
          <li key={l.to}>
            <Link to={l.to} className="text-[14px] text-sub transition hover:text-ink">{l.label}</Link>
          </li>
        ))}
      </ul>
    </div>
...
function CartButton() {
  ...
  const badge = count > 0 && (
    <span className="absolute -right-2 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-accent px-1 text-[11px] font-semibold text-white">
      {count}
    </span>
  );
...
function AccountMenu() {
  ...
  if (!user)
    return (
      <Link
        to="/login"
        className="text-[14px] tracking-tight text-sub transition-colors hover:text-ink"
      >
        เข้าสู่ระบบ
      </Link>
    );
      ...
      {open && (
        <div className="absolute right-0 mt-3 w-56 overflow-hidden rounded-2xl border border-line bg-white/90 shadow-lg backdrop-blur-xl">
          <div className="border-b border-line px-4 py-3">
            <p className="text-[15px] font-medium text-ink">{user.name || "สมาชิก"}</p>
            <p className="truncate text-[13px] text-sub">{user.email}</p>
          </div>
          <div className="py-1.5 text-[15px]">
