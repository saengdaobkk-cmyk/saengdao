  ...
  return (
    <section id={id} className="mx-auto max-w-page px-5 py-16 sm:py-20">
      ...
      <div className="mb-10">
        {q ? (
          <>
            <p className="text-[14px] font-medium tracking-tight text-sub">ผลการค้นหา</p>
          ...
          <>
            {eyebrow && <p className="text-[14px] font-medium tracking-tight text-sub">{eyebrow}</p>}
            <h2 className="mt-1 text-3xl font-semibold tracking-tightest text-ink sm:text-4xl">{heading}</h2>
            {data && showCollectionCount && <p className="mt-2 text-[14px] text-sub">{data.total} เล่ม</p>}
          </>
        )}
      </div>
      ...
      <div className="mb-10 flex items-center gap-3">
        <div className="relative min-w-0 flex-1">
          <div className="no-scrollbar flex gap-2 overflow-x-auto scroll-smooth py-0.5 pr-8">
            ...
            {publisher && (
              <span className="flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full bg-ink px-3.5 py-2 text-[14px] text-white">
        ...
        <>
          ...
          {data.totalPages > 1 && (
            <div className="mt-16 flex items-center justify-center gap-6">
              <button disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="text-[15px] text-sub transition hover:text-ink disabled:opacity-30">← ก่อนหน้า</button>
              <span className="text-[14px] tabular-nums text-sub">{data.page} / {data.totalPages}</span>
              <button disabled={page >= data.totalPages} onClick={() => setPage((p) => p + 1)} className="text-[15px] text-sub transition hover:text-ink disabled:opacity-30">ถัดไป →</button>
            </div>
          )}
        </>
      )}
...
function chip(active) {
  return `shrink-0 whitespace-nowrap rounded-full px-4 py-2 text-[14px] tracking-tight transition ${active ? "bg-ink text-white" : "bg-mist text-ink/70 hover:bg-line"}`;
}
