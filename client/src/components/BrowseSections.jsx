      ...
      {categories?.length > 0 && (
        <div>
          <div className="mb-6 flex items-center gap-4">
            ...
            <Link to="/books" className="shrink-0 rounded-full bg-ink px-5 py-2 text-[14px] font-medium text-white transition hover:bg-ink/90">
              ดูทั้งหมด →
            </Link>
          </div>
              ...
              {categories.map((c, i) => (
                ...
                >
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                    <p className="text-[19px] font-semibold text-white drop-shadow-sm">{c.name}</p>
                    <p className="text-[13px] text-white/80">{c.bookCount} เล่ม</p>
                  </div>
                </Link>
              ))}
            </div>
