      ...
      >
        ...
        {items.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center px-6 text-center">
            ...
            <p className="mt-4 text-[16px] text-sub">ตะกร้ายังว่างอยู่</p>
            <button
              onClick={closeDrawer}
              className="mt-6 rounded-full bg-accent px-6 py-2.5 text-[16px] font-medium text-white transition hover:bg-accent/90"
            >
              เลือกซื้อหนังสือ
            </button>
          </div>
          ...
          <>
                ...
                <li key={item.key} className="flex gap-3 py-4">
                  ...
                  <div className="flex flex-1 flex-col">
                    <Link
                      to={`/books/${item.id}`}
                      onClick={closeDrawer}
                      className="line-clamp-1 text-[16px] font-medium text-ink hover:text-accent"
                    >
                      {item.title}
                    </Link>
                    {item.variantName && <span className="text-[13px] text-ink/60">{item.variantName}</span>}
                    <p className="text-[14px] text-sub">{formatPrice(item.price)}</p>
                    <div className="mt-auto flex items-center justify-between pt-2">
                      <div className="flex items-center rounded-full border border-line">
                        <Qty onClick={() => setQty(item.key, item.quantity - 1)} disabled={item.quantity <= 1}>−</Qty>
                        <span className="w-7 text-center text-[15px] tabular-nums">{item.quantity}</span>
            ...
            <div className="border-t border-line px-5 py-4">
              ...
              <p className="mt-1 text-[14px] text-sub">ค่าจัดส่งฟรีทั่วประเทศ</p>
              <button
                onClick={goCheckout}
                className="mt-4 w-full rounded-full bg-accent py-3 text-[17px] font-medium text-white transition hover:bg-accent/90 active:scale-[0.99]"
              >
                ดำเนินการสั่งซื้อ
              </button>
              <Link
                to="/cart"
                onClick={closeDrawer}
                className="mt-2 block text-center text-[15px] text-sub hover:text-ink"
              >
                ดูตะกร้าแบบเต็ม
              </Link>
            </div>
...
function Qty({ children, onClick, disabled }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="flex h-7 w-7 items-center justify-center text-[17px] text-ink transition hover:bg-line/60 disabled:opacity-30"
    >
      {children}
    </button>
  );
