...
export default function FlipBook({ pdfUrl, title, open, onClose }) {
  ...
  const btn = "flex h-9 w-9 items-center justify-center rounded-full text-[20px] text-white/80 transition hover:bg-white/10 hover:text-white";

  return (
    <div className={`fixed inset-0 z-[100] flex-col bg-[#12161f] ${open ? "flex" : "hidden"}`}>
      {/* แถบบน */}
      <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
        <span className="flex min-w-0 items-center gap-2 text-[14px] text-white/90">
