...
function PromptPayBox({ orderId }) {
  ...
  return (
    <div className="rounded-2xl border border-line p-6 text-center">
      {isLoading && <p className="py-8 text-[15px] text-sub">กำลังสร้าง QR...</p>}
      {isError && <p className="py-8 text-[15px] text-red-500">สร้าง QR ไม่สำเร็จ (ร้านอาจยังไม่ตั้งค่าพร้อมเพย์)</p>}
      {data && (
        <>
          ...
          <button
            ...
            onClick={saveCard}
            disabled={saving}
            className="mt-4 inline-flex items-center gap-2 rounded-full border border-line px-5 py-2.5 text-[16px] font-medium text-ink transition hover:bg-mist disabled:opacity-50"
          >
          ...
          <p className="mt-2 text-[13px] text-sub">เซฟรูปแล้วเปิดในแอปธนาคาร → สแกนจากรูปได้เลย</p>
        </>
      )}
    </div>
  );
...
function Line({ label, value, copy, strong }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      ...
      <dd className={`flex items-center gap-2 text-right ${strong ? "text-[18px] font-semibold text-ink" : "text-ink"}`}>
        {value}
        {copy && value && (
          <button
            type="button"
            ...
            className="rounded-full bg-mist px-2 py-0.5 text-[13px] text-sub hover:text-ink"
          >
...
function Banner({ tone, title, desc }) {
  ...
  return (
    <div className={`rounded-2xl px-5 py-4 text-center ${tones[tone]}`}>
      ...
      <p className="mt-1 text-[15px] opacity-90">{desc}</p>
    </div>
  );
}
