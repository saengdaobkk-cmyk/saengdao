...
export default function PublisherMarquee() {
  ...
  return (
    <section className="mt-6 border-t border-line bg-white pb-6 pt-7">
      <p className="mb-5 text-center text-[15px] font-medium tracking-tight text-sub">{t("common.marquee_title", "สำนักพิมพ์ที่ร่วมกับเรา")}</p>
          ...
          {loop.map((p, i) => (
            ...
            >
              {p.image ? (
                ...
                <span className="whitespace-nowrap text-[19px] font-semibold tracking-tight text-ink/70">{p.name}</span>
              )}
            </Link>
          ))}
        </div>
