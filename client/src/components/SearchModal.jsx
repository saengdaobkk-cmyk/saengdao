        ...
        <div className="w-full max-w-xl overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
          ...
          <form onSubmit={(e) => { e.preventDefault(); goAll(); }} className="flex items-center gap-3 border-b border-line px-5">
            ...
            <input
              ...
              className="w-full bg-transparent py-4 text-[17px] text-ink outline-none placeholder:text-sub/70"
            />
            {term && (
            ...
            <button type="button" onClick={onClose} className="shrink-0 rounded-lg px-2 py-1 text-[13px] text-sub hover:text-ink">esc</button>
          </form>

          {/* ผลลัพธ์ */}
          <div className="max-h-[62vh] overflow-y-auto">
            {!dq && (
              <p className="px-5 py-10 text-center text-[14px] text-sub">พิมพ์เพื่อค้นหาหนังสือ · รองรับ ISBN</p>
            )}

            {dq && items.length === 0 && !isFetching && (
              <p className="px-5 py-10 text-center text-[14px] text-sub">ไม่พบผลลัพธ์สำหรับ “{dq}”</p>
            )}
                    ...
                    <li key={b.id}>
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p title={b.title} className="truncate text-[15px] font-medium text-ink">{b.title}</p>
                          <p
                            title={b.author + (b.translator ? ` · แปล ${b.translator}` : "")}
                            className="truncate text-[13.5px] text-sub"
                          >
                            {b.author}
                            {b.translator && ` · แปล ${b.translator}`}
                          </p>
                        </div>
                        <span className={`shrink-0 text-[14px] font-semibold ${pi.price < pi.original ? "text-rose-600" : "text-ink"}`}>
            ...
            {dq && total > items.length && (
              <button onClick={goAll} className="w-full border-t border-line px-5 py-3 text-left text-[14px] font-medium text-accent hover:bg-mist">
