...
export default function About() {
  return (
    <div className="mx-auto max-w-3xl px-5 py-16 sm:py-24">
      <p className="text-[14px] font-medium tracking-tight text-sub">เกี่ยวกับเรา</p>
      ...
      <div className="mt-12 grid gap-6 sm:grid-cols-3">
        {[
          ...
          <div key={x.t} className="rounded-2xl bg-mist p-6">
            <p className="text-[16px] font-semibold text-ink">{x.t}</p>
            <p className="mt-1.5 text-[15px] leading-relaxed text-sub">{x.d}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-12 flex flex-wrap gap-3">
        <Link to="/" className="rounded-full bg-accent px-7 py-3 text-[16px] font-medium text-white transition hover:bg-accent/90">
          เลือกซื้อหนังสือ
        </Link>
        <Link to="/contact" className="rounded-full border border-line px-7 py-3 text-[16px] font-medium text-ink transition hover:bg-mist">
          ติดต่อเรา
        </Link>
      </div>
    </div>
