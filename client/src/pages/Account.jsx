    ...
    <div className="mx-auto max-w-3xl px-5 py-12 sm:py-16">
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[15px] text-sub">{user.email}</p>
        </div>
      </div>
      ...
      <button
        onClick={logout}
        className="mt-12 text-[15px] text-sub transition hover:text-red-600"
      >
        ออกจากระบบ
      </button>
    </div>
...
function ProfileSection({ user, updateUser }) {
  ...
  return (
    <section className="rounded-2xl border border-line p-6">
      <h2 className="mb-4 text-[16px] font-semibold text-ink">ข้อมูลบัญชี</h2>
      <p className="mb-4 -mt-2 text-[13px] text-sub">ข้อมูลนี้จะถูกเติมให้อัตโนมัติตอนสั่งซื้อ</p>
      <form onSubmit={save} className="space-y-4">
        ...
        <label className="block">
          <span className="mb-1.5 block text-[14px] font-medium text-ink">ที่อยู่จัดส่ง</span>
          <textarea
            value={form.address}
            onChange={(e) => set("address")(e.target.value)}
            rows={3}
            className="w-full resize-none rounded-xl border border-line bg-white px-4 py-2.5 text-[16px] text-ink outline-none transition focus:border-ink/30"
          />
        </label>
        <div className="flex items-center gap-4">
          <button type="submit" disabled={busy} className="rounded-full bg-ink px-6 py-2.5 text-[15px] font-medium text-white transition hover:bg-ink/90 disabled:opacity-50">
            {busy ? "กำลังบันทึก..." : "บันทึก"}
          </button>
          {msg && <span className={`text-[14px] ${msg === "บันทึกแล้ว" ? "text-emerald-600" : "text-red-600"}`}>{msg}</span>}
        </div>
      </form>
    </section>
  );
...
function ReceiptAddressSection({ user, updateUser }) {
  ...
  return (
    <section className="rounded-2xl border border-line p-6">
      <h2 className="mb-1 text-[16px] font-semibold text-ink">ที่อยู่ออกใบเสร็จ</h2>
      <p className="mb-4 text-[13px] text-sub">สำหรับใบเสร็จ/ใบกำกับภาษี — เว้นว่างได้ถ้าใช้ที่อยู่จัดส่ง</p>
      <form onSubmit={save} className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="block">
          <span className="mb-1.5 block text-[14px] font-medium text-ink">ที่อยู่ออกใบเสร็จ</span>
          <textarea
...
function PasswordSection() {
  ...
  return (
    <section className="rounded-2xl border border-line p-6">
      <h2 className="mb-4 text-[16px] font-semibold text-ink">เปลี่ยนรหัสผ่าน</h2>
      <form onSubmit={save} className="space-y-4">
        ...
        <div className="flex items-center gap-4">
          <button type="submit" disabled={busy || !cur || !next} className="rounded-full bg-ink px-6 py-2.5 text-[15px] font-medium text-white transition hover:bg-ink/90 disabled:opacity-50">
            {busy ? "กำลังเปลี่ยน..." : "เปลี่ยนรหัสผ่าน"}
          </button>
          {msg && <span className={`text-[14px] ${msg.includes("แล้ว") ? "text-emerald-600" : "text-red-600"}`}>{msg}</span>}
        </div>
      </form>
    </section>
  );
...
function OrdersSection() {
  ...
  return (
    <section>
      <h2 className="mb-4 text-[16px] font-semibold text-ink">ประวัติคำสั่งซื้อ</h2>
      {isLoading ? (
        <p className="text-sub">กำลังโหลด...</p>
      ) : !orders?.length ? (
        <div className="rounded-2xl border border-line p-8 text-center">
          <p className="text-[15px] text-sub">ยังไม่มีคำสั่งซื้อ</p>
          <Link to="/" className="mt-3 inline-block text-[15px] text-accent">เลือกซื้อหนังสือ</Link>
        </div>
      ) : (
              ...
              >
                <div className="min-w-0 flex-1">
                  <p className="text-[15px] font-medium text-ink">
                    ...
                    <span className="ml-2 text-[13px] font-normal text-sub">
                ...
                <span className={`shrink-0 rounded-full px-2.5 py-1 text-[12px] font-medium ${st.cls}`}>{st.label}</span>
                <span className="w-20 shrink-0 text-right text-[15px] font-semibold text-ink">{formatPrice(o.total)}</span>
              </Link>
            );
          })}
        </div>
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="w-full rounded-xl border border-line bg-white px-4 py-2.5 text-[16px] text-ink outline-none transition focus:border-ink/30"
      />
    </label>
  );
}
