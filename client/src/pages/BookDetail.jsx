...
export default function BookDetail() {
  ...
  if (isError || !book)
    return (
      <div className="mx-auto max-w-page px-5 py-32 text-center">
        ...
        <Link to="/books" className="text-[16px] text-accent">{t("product.back_to_shop", "กลับไปร้านหนังสือ")}</Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-page px-5 py-10 sm:py-14">
      {/* breadcrumbs */}
      <nav className="flex flex-wrap items-center gap-1.5 text-[14px] text-sub">
      ...
      <div className="mt-6 grid gap-12 md:grid-cols-[minmax(0,380px)_1fr] md:gap-16">
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {book.publisherLink && <Link to={`/publisher/${book.publisherLink.slug}`} className="inline-flex items-center gap-1.5 rounded-full border border-line px-3 py-1 text-[14px] text-ink transition hover:border-ink/40"><BuildingIcon />{book.publisherLink.name}</Link>}
            {book.category && <Link to={`/books?category=${book.category.slug}`} className="rounded-full bg-mist px-3 py-1 text-[14px] text-ink/70 transition hover:bg-line">{book.category.name}</Link>}
            {(book.tags || []).map((t) => <span key={t} className="rounded-full bg-mist px-2.5 py-1 text-[13px] text-ink/60">{t}</span>)}
          </div>
          ...
          {book.translatorLinks?.length > 0 && (
            <p className="mt-0.5 text-[16px] text-sub">
          ...
          <div className={`mt-6 rounded-2xl border p-5 ${isHot ? "border-orange-200 bg-orange-50/50" : "border-line"}`}>
            {isHot && (
              <p className="mb-2 inline-flex items-center gap-1.5 rounded-full bg-orange-500 px-2.5 py-1 text-[14px] font-semibold text-white">
            ...
            <div className="flex items-end gap-3">
              ...
              {showDiscount && (
                <>
                  <span className="pb-1 text-[18px] text-sub line-through">{formatPrice(showFull)}</span>
                  <span className={`mb-1.5 rounded-full px-2 py-0.5 text-[14px] font-semibold text-white ${isHot ? "bg-orange-500" : "bg-rose-500"}`}>{t("product.discount_label", "ลด")} {pct}%</span>
                </>
              )}
            </div>
            ...
            {hasVariants && (
              <div className="mt-4">
                <p className="mb-2 text-[15px] font-medium text-ink">{t("product.options_heading", "ตัวเลือก")} {variantErr && <span className="text-rose-500">· {t("product.select_first", "กรุณาเลือกก่อน")}</span>}</p>
                  ...
                  {book.variants.map((v) => {
                    ...
                    return (
                      <button key={v.id} disabled={out} onClick={() => { setVariantId(v.id); setVariantErr(false); setQty(1); }}
                        className={`rounded-xl border px-4 py-2 text-[15px] transition ${active ? "border-ink bg-ink text-white" : "border-line text-ink hover:border-ink/40"} ${out ? "cursor-not-allowed opacity-40" : ""}`}>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              <div className="flex items-center rounded-full border border-line">
                <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="flex h-10 w-10 items-center justify-center text-[20px] text-ink hover:bg-mist">−</button>
                <span className="w-10 text-center text-[17px] tabular-nums">{qty}</span>
                <button onClick={() => setQty((q) => (effStock != null ? Math.min(effStock, q + 1) : q + 1))} className="flex h-10 w-10 items-center justify-center text-[20px] text-ink hover:bg-mist">+</button>
              </div>
              <button disabled={effStock != null && effStock <= 0} onClick={() => addToCart(false)}
                className="flex-1 rounded-full bg-accent px-8 py-3 text-[17px] font-medium text-white transition hover:bg-accent/90 active:scale-[0.98] disabled:opacity-40">
                {t("product.add_to_cart", "หยิบใส่ตะกร้า")}
              </button>
              {book.previewPdf && (
                <button type="button" onClick={() => setFlipOpen(true)}
                  className="flex w-full items-center justify-center gap-2 rounded-full border border-line px-8 py-3 text-[17px] font-medium text-ink transition hover:bg-mist active:scale-[0.98] sm:w-auto">
            ...
            {/* trust */}
            <div className="mt-5 grid grid-cols-2 gap-3 border-t border-line pt-4 text-[15px] text-sub">
          ...
          {book.description && (
            <div className="mt-8">
              <h2 className="mb-3 text-[17px] font-semibold tracking-tight text-ink">{t("product.description_heading", "รายละเอียด")}</h2>
              <p className={`whitespace-pre-line text-[17px] leading-relaxed text-ink/80 ${!descOpen && longDesc ? "line-clamp-[7]" : ""}`}>
                {book.description}
              </p>
              {longDesc && (
                <button onClick={() => setDescOpen((o) => !o)} className="mt-2 text-[16px] font-medium text-accent">
          ...
          {meta.length > 0 && (
            <div className="mt-8 border-t border-line pt-8">
              <h2 className="mb-3 text-[17px] font-semibold tracking-tight text-ink">{t("product.specs_heading", "ข้อมูลจำเพาะ")}</h2>
              <dl className="grid gap-x-8 gap-y-2 text-[16px] sm:grid-cols-2">
                {meta.map(([k, v]) => (
      ...
      {related?.length > 0 && (
        <section className="mt-20">
          <p className="text-[15px] font-medium tracking-tight text-sub">{t("product.related_eyebrow", "เล่มใกล้เคียง")}</p>
      ...
      {lbIndex != null && (
        <div onClick={() => setLbIndex(null)} className="fixed inset-0 z-[90] flex items-center justify-center bg-ink/80 p-6 backdrop-blur-sm">
          ...
          {allImages.length > 1 && (
            <span className="absolute bottom-6 left-1/2 -translate-x-1/2 rounded-full bg-white/15 px-3 py-1 text-[15px] text-white backdrop-blur">
