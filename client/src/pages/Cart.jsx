  ...
  if (items.length === 0)
    return (
      <div className="mx-auto max-w-page px-5 py-24 text-center">
        ...
        <Link
          to="/"
          className="mt-8 inline-block rounded-full bg-accent px-7 py-3 text-[15px] font-medium text-white transition hover:bg-accent/90"
        >
    ...
    <div className="mx-auto max-w-page px-5 py-12 sm:py-16">
      ...
      <div className="grid gap-10 lg:grid-cols-[1fr_360px]">
          ...
          {items.map((item) => (
            <li key={item.key} className="flex gap-4 py-5">
              ...
              <div className="flex flex-1 flex-col">
                ...
                {item.variantName && (
                  <span className="mt-0.5 w-fit rounded-full bg-mist px-2 py-0.5 text-[11px] text-ink/70">{item.variantName}</span>
                )}
                <p className="text-[13px] text-sub">{item.author}</p>

                <div className="mt-auto flex items-center justify-between pt-3">
                  {/* ตัวปรับจำนวน */}
                  <div className="flex items-center rounded-full border border-line">
                    <QtyBtn onClick={() => setQty(item.key, item.quantity - 1)} disabled={item.quantity <= 1}>−</QtyBtn>
                    <span className="w-8 text-center text-[14px] tabular-nums">{item.quantity}</span>
        ...
        <aside className="h-fit rounded-2xl border border-line bg-mist/50 p-6 lg:sticky lg:top-20">
          <h2 className="text-[15px] font-semibold text-ink">สรุปคำสั่งซื้อ</h2>
          <div className="mt-4 flex justify-between text-[14px] text-sub">
            <span>ยอดรวมสินค้า</span>
            <span className="text-ink">{formatPrice(subtotal)}</span>
          </div>
          <div className="mt-2 flex justify-between text-[14px] text-sub">
            <span>ค่าจัดส่ง</span>
          ...
          <button
            onClick={goCheckout}
            className="mt-6 w-full rounded-full bg-accent py-3 text-[15px] font-medium text-white transition hover:bg-accent/90 active:scale-[0.99]"
          >
            ดำเนินการสั่งซื้อ
          </button>
          <Link to="/" className="mt-3 block text-center text-[13px] text-sub hover:text-ink">
            เลือกซื้อต่อ
          </Link>
        </aside>
      </div>
...
function QtyBtn({ children, onClick, disabled }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="flex h-8 w-8 items-center justify-center text-[16px] text-ink transition hover:bg-line/60 disabled:opacity-30"
    >
      {children}
    </button>
  );
