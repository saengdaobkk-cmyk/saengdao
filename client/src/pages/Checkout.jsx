    ...
    <div className="mx-auto max-w-page px-5 py-12 sm:py-16">
      ...
      <form onSubmit={submit} className="grid gap-10 lg:grid-cols-[1fr_360px]">
        <div className="space-y-10">
          {/* ที่อยู่จัดส่ง */}
          <section>
            <h2 className="mb-4 text-[18px] font-semibold text-ink">{t("checkout.shipping_heading", "ที่อยู่จัดส่ง")}</h2>
          ...
          {shippingMethods.length > 0 && (
            <section>
              <h2 className="mb-4 text-[18px] font-semibold text-ink">{t("checkout.shipping_method_heading", "ช่องทางจัดส่ง")}</h2>
                ...
                {shippingMethods.map((m) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className="text-[17px] font-medium text-ink">{m.name}</p>
                      {m.note && <p className="text-[15px] text-sub">{m.note}</p>}
                    </div>
          ...
          <section>
            <label className="flex cursor-pointer items-center gap-3">
              ...
              />
              <span className="text-[18px] font-semibold text-ink">{t("checkout.receipt_toggle", "ออกใบเสร็จรับเงิน / ใบกำกับภาษี")}</span>
            </label>
          ...
          {/* วิธีชำระเงิน */}
          <section>
            <h2 className="mb-4 text-[18px] font-semibold text-ink">{t("checkout.payment_heading", "วิธีชำระเงิน")}</h2>
            <div className="space-y-3">
              {PAYMENTS.map((p) => (
                <label
                  key={p.value}
                ...
                >
                  ...
                  />
                  <div>
                    <p className="text-[17px] font-medium text-ink">{p.label}</p>
                    <p className="text-[15px] text-sub">{p.desc}</p>
                  </div>
                </label>
              ))}
            </div>
          ...
          {/* หมายเหตุ */}
          <section>
            <h2 className="mb-3 text-[18px] font-semibold text-ink">{t("checkout.note_heading", "หมายเหตุถึงร้าน")}</h2>
            <textarea
              value={note}
              onChange={(e) => setNote(e.target.value)}
              rows={3}
              placeholder={t("checkout.note_placeholder", "ระบุรายละเอียดเพิ่มเติม เช่น เวลาที่สะดวกรับของ (ถ้ามี)")}
              className="w-full resize-none rounded-xl border border-line bg-white px-4 py-2.5 text-[18px] text-ink outline-none transition placeholder:text-sub/70 focus:border-ink/30"
            />
          </section>
        </div>
        ...
        <aside className="h-fit rounded-2xl border border-line bg-mist/50 p-6 lg:sticky lg:top-20">
          <h2 className="text-[18px] font-semibold text-ink">{t("checkout.summary_heading", "คำสั่งซื้อ")}</h2>
          ...
          <div className="mt-5 border-t border-line pt-5">
            {coupon ? (
              <div className="flex items-center justify-between rounded-xl bg-emerald-50 px-3 py-2.5">
                ...
                <button type="button" onClick={removeCoupon} className="text-[15px] text-emerald-700">
                  ยกเลิก
                </button>
              </div>
            ) : (
              <>
                ...
                {couponError && <p className="mt-2 text-[15px] text-red-500">{couponError}</p>}
              </>
            )}
          </div>
          ...
          <div className="mt-4 flex justify-between border-t border-line pt-4 text-[19px] font-semibold text-ink">
            ...
            <div className="mt-4">
              ...
              {error.includes("ตะกร้า") && (
                <button
                  ...
                  className="mt-2 text-[16px] font-medium text-accent"
                >
          ...
          <button
            type="submit"
            disabled={busy}
            className="mt-6 w-full rounded-full bg-accent py-3 text-[18px] font-medium text-white transition hover:bg-accent/90 active:scale-[0.99] disabled:opacity-50"
          >
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        required={required}
        {...rest}
        className={`w-full rounded-xl border bg-white px-4 py-2.5 text-[18px] text-ink outline-none transition placeholder:text-sub/60 ${
          error ? "neon-error" : "border-line focus:border-ink/30"
        }`}
      />
      {error && <span className="mt-1.5 block text-[15px] text-red-500">{error}</span>}
    </label>
  );
}
  ...
  return (
    <label className="block">
      ...
      <textarea
        ...
        required={required}
        rows={3}
        className={`w-full resize-none rounded-xl border bg-white px-4 py-2.5 text-[18px] text-ink outline-none transition ${
          error ? "neon-error" : "border-line focus:border-ink/30"
        }`}
      />
      {error && <span className="mt-1.5 block text-[15px] text-red-500">{error}</span>}
