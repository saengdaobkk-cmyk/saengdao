...
export default function Contact() {
  ...
  return (
    <div className="mx-auto max-w-page px-5 py-16 sm:py-24">
      <p className="text-[16px] font-medium tracking-tight text-sub">ติดต่อเรา</p>
      <h1 className="mt-2 text-4xl font-semibold tracking-tightest text-ink sm:text-5xl">ยินดีให้บริการ</h1>
      <p className="mt-4 max-w-xl text-[19px] leading-relaxed text-sub">
      ...
      <div className="mt-12 grid gap-10 lg:grid-cols-[1fr_1.2fr]">
        ...
        <div className="space-y-5">
          ...
          {!CONTACT.phone && !CONTACT.email && !CONTACT.address && (
            <p className="text-[17px] text-sub">ยังไม่ได้ตั้งค่าข้อมูลติดต่อ — เพิ่มได้ที่ จัดการร้าน → ตั้งค่า</p>
          )}
        </div>

        {/* ฟอร์ม */}
        <div className="rounded-2xl border border-line p-6 sm:p-8">
          <h2 className="text-[18px] font-semibold text-ink">ส่งข้อความถึงเรา</h2>
          <form onSubmit={submit} className="mt-4 space-y-4">
            ...
            <label className="block">
              <span className="mb-1.5 block text-[16px] font-medium text-ink">ข้อความ</span>
              <textarea value={form.message} onChange={set("message")} rows={5} required
                className="w-full resize-none rounded-xl border border-line px-4 py-2.5 text-[18px] text-ink outline-none focus:border-ink/30" />
            </label>
            <button type="submit" className="rounded-full bg-accent px-7 py-3 text-[18px] font-medium text-white transition hover:bg-accent/90">
              ส่งข้อความ
            </button>
            {sent && <p className="text-[16px] text-emerald-600">กำลังเปิดแอปอีเมลให้คุณส่ง…</p>}
          </form>
        </div>
      </div>
    </div>
...
function Info({ icon, label, value, href }) {
  const inner = (
    <>
      ...
      <span>
        <span className="block text-[15px] text-sub">{label}</span>
        <span className="text-[18px] text-ink">{value}</span>
      </span>
    </>
  );
  return href ? (
  ...
  return (
    <label className="block">
      ...
      <input type={type} value={value} onChange={onChange} required
        className="w-full rounded-xl border border-line px-4 py-2.5 text-[18px] text-ink outline-none focus:border-ink/30" />
    </label>
  );
}
