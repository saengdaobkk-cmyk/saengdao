...
export default function Home() {
  ...
  // เปลี่ยนหมวดเมื่อกดเมนูบนแถบ nav (?category=...)
  useEffect(() => {
  ...
  return (
    <>
      ...
      <section id="catalog" className="mx-auto max-w-page px-5 py-16 sm:py-24">
        {/* หัวเซกชัน */}
        <div className="mb-10 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-[13px] font-medium tracking-tight text-sub">คอลเลกชัน</p>
            <h2 className="mt-1 text-3xl font-semibold tracking-tightest text-ink sm:text-4xl">
              คัดสรรมาเพื่อคุณ
            </h2>
          </div>
          ...
          <form onSubmit={submitSearch} className="relative w-full sm:w-72">
            ...
            <button
              type="submit"
              aria-label="ค้นหา"
              className="absolute right-1.5 top-1/2 -translate-y-1/2 rounded-full p-2 text-sub hover:text-ink"
            >
        ...
        <div className="mb-10 flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap gap-2">
            <button onClick={() => pickCategory("")} className={chip(category === "")}>
              ทั้งหมด
            </button>
            {categories?.map((c) => (
              <button key={c.id} onClick={() => pickCategory(c.slug)} className={chip(category === c.slug)}>
                {c.name}
              </button>
            ))}
          </div>
        ...
        {data && data.items.length === 0 && (
          <p className="py-20 text-center text-sub">ไม่พบหนังสือที่ค้นหา</p>
        )}
            ...
            {data.totalPages > 1 && (
              <div className="mt-16 flex items-center justify-center gap-6">
                <button
                  ...
                  className="text-[14px] text-sub transition hover:text-ink disabled:opacity-30"
                >
                  ← ก่อนหน้า
                </button>
...
function chip(active) {
  return `rounded-full px-4 py-2 text-[13px] tracking-tight transition ${
    active
      ? "bg-ink text-white"
    ...
    <div className="grid grid-cols-2 gap-x-5 gap-y-10 sm:grid-cols-3 lg:grid-cols-4">
      {Array.from({ length: 8 }).map((_, i) => (
        <div key={i} className="animate-pulse">
          ...
          <div className="mt-2 h-3.5 w-1/4 rounded bg-mist" />
        </div>
      ))}
    </div>
  );
