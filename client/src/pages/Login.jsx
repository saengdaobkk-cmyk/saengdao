  ...
  return (
    <AuthShell title="เข้าสู่ระบบ" subtitle="ยินดีต้อนรับกลับสู่ SAENGDAO">
      {location.state?.from === "/checkout" && (
        <p className="mb-5 rounded-xl bg-mist px-4 py-3 text-center text-[16px] text-ink">
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <button
          type="submit"
          disabled={busy}
          className="w-full rounded-full bg-accent py-3 text-[18px] font-medium text-white transition hover:bg-accent/90 active:scale-[0.99] disabled:opacity-50"
        >
          {busy ? "กำลังเข้าสู่ระบบ..." : "เข้าสู่ระบบ"}
        </button>
      </form>

      <p className="mt-6 text-center text-[16px] text-sub">
        ยังไม่มีบัญชี?{" "}
...
export function AuthShell({ title, subtitle, children }) {
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-sm flex-col justify-center px-5 py-16">
      ...
      {subtitle && <p className="mb-8 mt-2 text-center text-[17px] text-sub">{subtitle}</p>}
      {children}
    </div>
  );
}
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        required
        className={`w-full rounded-xl border bg-mist px-4 py-3 text-[18px] text-ink outline-none transition focus:bg-white ${
