  ...
  return (
    <div className="mx-auto max-w-page px-5">
      ...
      <div className="border-b border-line py-10 text-center sm:py-14">
        ...
        <p className="mx-auto mt-2 max-w-md text-[17px] text-sub">
          {result
      ...
      <div className="mx-auto max-w-md py-12">
        {result ? (
          <>
            <button
              onClick={back}
              className="mb-5 inline-flex items-center gap-1.5 text-[17px] text-sub transition hover:text-ink"
            >
          ...
          <div className="rounded-2xl border border-line p-6">
            <h3 className="text-[18px] font-semibold text-ink">ค้นหาคำสั่งซื้อของคุณ</h3>
            {lookup.isError && (
              <div className="mt-4 rounded-xl bg-red-50 px-4 py-3 text-[16px] text-red-600">
                {lookup.error?.response?.data?.error || "เกิดข้อผิดพลาด ลองใหม่อีกครั้ง"}
              </div>
            )}
            <form onSubmit={submit} className="mt-5 space-y-4">
              <label className="block">
                <span className="text-[16px] font-medium text-ink">เลขคำสั่งซื้อ *</span>
                <input
                  value={code}
                  onChange={(e) => setCode(e.target.value)}
                  placeholder="เช่น 0DCCAE98"
                  required
                  className="mt-1.5 w-full rounded-xl border border-line px-4 py-2.5 text-[17px] text-ink outline-none transition focus:border-accent"
                />
              </label>
              <label className="block">
                <span className="text-[16px] font-medium text-ink">เบอร์โทรศัพท์ *</span>
                <input
                  value={phone}
              ...
              <button
                ...
                className="w-full rounded-full bg-accent py-3 text-[18px] font-medium text-white transition hover:bg-accent/90 disabled:opacity-50"
              >
  ...
  return (
    <div className="mt-8 rounded-2xl border border-line p-6">
      <div className="flex items-center justify-between gap-3">
        <p className="text-[16px] text-sub">เลขคำสั่งซื้อ</p>
        <span className="font-mono text-[17px] font-semibold text-ink">
          #{order.id.slice(0, 8).toUpperCase()}
        </span>
      </div>
      <div className="mt-3 flex items-center justify-between gap-3">
        <p className="text-[16px] text-sub">สถานะ</p>
        <span className={`rounded-full px-3 py-1 text-[15px] font-semibold ${STATUS_TONE[order.paymentStatus] || "bg-mist text-ink"}`}>
          {displayStatus(order)}
        </span>
      </div>
      ...
      <dl className="mt-5 space-y-1.5 text-[16px]">
      ...
      {order.paymentStatus === "PENDING_REVIEW" && (
        <div className="mt-6 rounded-xl bg-amber-50 px-5 py-4 text-center text-amber-700">
          <p className="text-[17px] font-semibold">ได้รับสลิปแล้ว กำลังตรวจสอบ</p>
          <p className="mt-1 text-[15px] opacity-90">ร้านจะยืนยันการชำระเงินภายใน 24 ชม.</p>
          {order.slipImage && (
      ...
      {order.paymentStatus === "PAID" && (
        <div className="mt-6 rounded-xl bg-emerald-50 px-5 py-4 text-center text-emerald-700">
          <p className="text-[17px] font-semibold">ชำระเงินเรียบร้อย</p>
          <p className="mt-1 text-[15px] opacity-90">ขอบคุณครับ ร้านกำลังจัดเตรียมสินค้าให้คุณ</p>
        </div>
      )}
