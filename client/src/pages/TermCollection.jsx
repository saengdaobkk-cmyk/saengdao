  ...
  if (isError || !term)
    return (
      <div className="mx-auto max-w-page px-5 py-32 text-center">
        ...
        <Link to="/books" className="text-[17px] text-accent">กลับไปร้านหนังสือ</Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-page px-5 py-12 sm:py-16">
      ...
      <div className="mb-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-[16px] font-medium tracking-tight text-sub">{TYPE_LABEL[type]}</p>
          <h1 className="mt-1 text-3xl font-semibold tracking-tightest text-ink sm:text-4xl">{term.name}</h1>
          {data && <p className="mt-2 text-[16px] text-sub">{data.total} เล่ม</p>}
        </div>
        <select value={sort} onChange={(e) => { setSort(e.target.value); setPage(1); }}
          className="rounded-full border border-line bg-white px-4 py-2 text-[16px] text-ink outline-none focus:border-ink/30">
