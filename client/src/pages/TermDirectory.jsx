...
export default function TermDirectory({ type }) {
  ...
  return (
    <div className="mx-auto max-w-page px-5 py-12 sm:py-16">
      <nav className="mb-6 flex flex-wrap items-center gap-1.5 text-[15px] text-sub">
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-[16px] font-medium tracking-tight text-sub">{meta.desc}</p>
          <h1 className="mt-1 text-3xl font-semibold tracking-tightest text-ink sm:text-4xl">{meta.title}</h1>
          {items && <p className="mt-2 text-[16px] text-sub">{items.length} {meta.label}</p>}
        </div>
        <input
          value={q}
          onChange={(e) => setQ(e.target.value)}
          placeholder={`ค้นหา${meta.label}...`}
          className="w-full rounded-full border border-line bg-white px-5 py-2.5 text-[17px] text-ink outline-none focus:border-ink/30 sm:w-72"
        />
      </div>
          ...
          {list.map((t) => (
            ...
            >
              <span className="min-w-0">
                <span className="block truncate text-[18px] font-medium text-ink transition-colors group-hover:text-accent">{t.name}</span>
                <span className="text-[15px] text-sub">{t.count} เล่ม</span>
              </span>
