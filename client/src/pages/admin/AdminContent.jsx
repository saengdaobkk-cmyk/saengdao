    ...
    <div className="grid gap-6 lg:grid-cols-[200px_1fr]">
      ...
      <aside className="flex gap-2 overflow-x-auto lg:flex-col lg:gap-1">
        {sections.map((s, idx) => (
          <button
            ...
            className={`whitespace-nowrap rounded-xl px-4 py-2.5 text-left text-[14px] transition ${
      ...
      <div className="rounded-2xl border border-line bg-white p-6">
        <div className="mb-5 flex items-center justify-between">
          <h2 className="text-[15px] font-semibold text-ink">{section.label}</h2>
          <p className="text-[12px] text-sub">{section.items.length} ข้อความ</p>
        </div>
          ...
          {section.items.map((item) => {
            ...
            return (
              <label key={item.key} className="block">
                <span className="mb-1.5 block text-[13px] font-medium text-ink">{item.label}</span>
                {long ? (
                  <textarea
                    value={v}
                ...
                <span className="mt-1 block text-[11px] text-sub/70">{item.key}</span>
              </label>
            );
          })}
        </div>
        ...
        <div className="mt-6 flex items-center gap-4">
          ...
          {savedFor === section.key && <span className="text-[13px] text-emerald-600">บันทึกแล้ว — หน้าร้านอัปเดตทันที</span>}
        </div>
      </div>
    </div>
  );
