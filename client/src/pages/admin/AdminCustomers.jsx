    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <p className="text-[14px] text-sub">ลูกค้าทั้งหมด {customers && `(${customers.length})`}</p>
      ...
      <div className="h-fit rounded-2xl border border-line bg-white p-5 lg:sticky lg:top-20">
        {editing ? (
          <form onSubmit={submit} className="space-y-3">
            <p className="text-[14px] font-semibold text-ink">แก้ไขลูกค้า</p>
            ...
            <div className="border-t border-line pt-3">
              <p className="mb-2 text-[13px] font-medium text-sub">ข้อมูลใบเสร็จ/ใบกำกับภาษี</p>
            ...
            <div className="flex gap-2 pt-1">
              <button type="submit" disabled={save.isPending} className="rounded-full bg-accent px-5 py-2 text-[14px] font-medium text-white hover:bg-accent/90 disabled:opacity-50">บันทึก</button>
              <button type="button" onClick={cancel} className="rounded-full border border-line px-4 py-2 text-[14px] text-ink hover:bg-mist">ยกเลิก</button>
            </div>
          </form>
        ) : (
          <p className="py-6 text-center text-[14px] text-sub">เลือกลูกค้าจากตาราง<br />แล้วกด "แก้ไข" เพื่อดู/แก้ข้อมูล</p>
        )}
      </div>
    </div>
  );
...
function Field({ label, value, onChange, type = "text", textarea }) {
  return (
    <label className="block">
      <span className="mb-1 block text-[13px] text-sub">{label}</span>
      {textarea ? (
        <textarea value={value} onChange={onChange} rows={2}
          className="w-full resize-none rounded-lg border border-line bg-white px-3 py-2 text-[15px] outline-none focus:border-ink/30" />
      ) : (
        <input type={type} value={value} onChange={onChange}
          className="w-full rounded-lg border border-line bg-white px-3 py-2 text-[15px] outline-none focus:border-ink/30" />
      )}
    </label>
  );
}
