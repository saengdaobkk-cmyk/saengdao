                ...
                <tr key={c.id} className="hover:bg-mist/30">
                  ...
                  <td className="whitespace-nowrap px-5 py-3">
                    <div className="flex justify-end gap-4">
                      <button onClick={() => startEdit(c)} className="text-[13px] text-accent">แก้ไข</button>
                      <button
      ...
      <div className="h-fit rounded-2xl border border-line bg-white p-5 lg:sticky lg:top-20">
        {editing ? (
          <form onSubmit={submit} className="space-y-3">
            <p className="text-[13px] font-semibold text-ink">แก้ไขลูกค้า</p>
            ...
            <div className="flex gap-2 pt-1">
              <button type="submit" disabled={save.isPending} className="rounded-full bg-accent px-5 py-2 text-[13px] font-medium text-white hover:bg-accent/90 disabled:opacity-50">บันทึก</button>
              <button type="button" onClick={cancel} className="rounded-full border border-line px-4 py-2 text-[13px] text-ink hover:bg-mist">ยกเลิก</button>
            </div>
          </form>
        ) : (
          <p className="py-6 text-center text-[13px] text-sub">เลือกลูกค้าจากตาราง<br />แล้วกด "แก้ไข" เพื่อดู/แก้ข้อมูล</p>
        )}
      </div>
    </div>
  );
