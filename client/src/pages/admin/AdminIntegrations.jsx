...
export default function AdminIntegrations() {
  ...
  return (
    <div className="space-y-6">
      <p className="text-[14px] text-sub">เชื่อม SAENGDAO กับแอปภายนอกผ่าน API</p>
      ...
      <div className="rounded-2xl border border-dashed border-line bg-white p-6 text-center">
        <p className="text-[15px] font-medium text-ink">Shopee · Lazada · LINE Shopping</p>
        <p className="mt-1 text-[13px] text-sub">เร็วๆ นี้</p>
      </div>
    </div>
  );
}
    ...
    <div className="rounded-2xl border border-line bg-white p-6">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-ink text-[14px] font-bold text-white">Z</div>
          <div>
            <p className="text-[16px] font-semibold text-ink">ZORT</p>
            <p className="text-[13px] text-sub">ระบบจัดการออเดอร์ & สต็อก</p>
          </div>
        </div>
        <span className={`rounded-full px-2.5 py-1 text-[12px] font-medium ${zort.connected ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1">
          <button type="submit" disabled={save.isPending} className="rounded-full bg-ink px-6 py-2.5 text-[15px] font-medium text-white transition hover:bg-ink/90 disabled:opacity-50">
            {save.isPending ? "กำลังบันทึก..." : "บันทึก"}
          </button>
          <button type="button" onClick={runTest} disabled={testing} className="rounded-full border border-line px-5 py-2.5 text-[15px] font-medium text-ink transition hover:bg-mist disabled:opacity-50">
            {testing ? "กำลังทดสอบ..." : "ทดสอบการเชื่อมต่อ"}
          </button>
          {savedMsg && <span className="text-[14px] text-emerald-600">{savedMsg}</span>}
        </div>

        {test && (
          <p className={`text-[14px] ${test.ok ? "text-emerald-600" : "text-red-600"}`}>
      ...
      <div className="mt-5 border-t border-line pt-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-[14px] font-medium text-ink">สต็อกจาก ZORT</p>
          ...
          <button
            ...
            className="shrink-0 rounded-full border border-line px-4 py-2 text-[14px] font-medium text-ink transition hover:bg-mist disabled:opacity-50"
          >
            {syncing ? "กำลังดึง..." : "ดึงสต็อกตอนนี้"}
          </button>
        </div>
        {syncMsg && <p className={`mt-2 text-[13px] ${syncMsg.ok ? "text-emerald-600" : "text-red-600"}`}>{syncMsg.text}</p>}
      </div>

      <p className="mt-4 border-t border-line pt-4 text-[13px] text-sub">
...
function ThaipostCard({ thpost }) {
  ...
  return (
    <div className="rounded-2xl border border-line bg-white p-6">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#ED1C24] text-[16px] font-bold text-white">฿</div>
          <div>
            <p className="text-[16px] font-semibold text-ink">ไปรษณีย์ไทย</p>
            <p className="text-[13px] text-sub">ติดตามสถานะพัสดุ (Track &amp; Trace)</p>
          </div>
        </div>
        <span className={`rounded-full px-2.5 py-1 text-[12px] font-medium ${thpost.connected ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="w-full rounded-xl border border-line px-4 py-2.5 text-[15px] text-ink outline-none transition placeholder:text-sub/50 focus:border-ink/30"
      />
    </label>
  );
}
