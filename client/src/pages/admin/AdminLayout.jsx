    ...
    <div className="min-h-screen bg-mist text-ink">
      ...
      >
        <div className="flex h-14 items-center gap-2 border-b border-line px-5">
          <span className="text-[18px] font-semibold tracking-[0.18em]">SAENGDAO</span>
          <span className="rounded bg-ink px-1.5 py-0.5 text-[13px] font-medium tracking-wide text-white">{isAdmin ? "ADMIN" : "STAFF"}</span>
        </div>
        ...
        <div className="space-y-1 border-t border-line p-3">
          ...
          <Link
            to="/"
            className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-[17px] text-sub transition hover:bg-mist hover:text-ink"
          >
            <StoreIcon />
            ดูหน้าร้าน
          </Link>
...
function SideLink({ item, onClick }) {
  ...
  return (
    ...
    >
      ...
      {count > 0 && (
        <span className="rounded-full bg-amber-500 px-1.5 text-[14px] font-semibold text-white">{count}</span>
      )}
    </NavLink>
  );
}
...
function AccountBox() {
  ...
  return (
    <div className="mt-1 flex items-center gap-2 rounded-xl px-3 py-2">
      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-mist text-[16px] font-medium text-ink">
        {(user.name || user.email)[0].toUpperCase()}
      </div>
      <div className="min-w-0 flex-1">
        <p className="truncate text-[15px] font-medium text-ink">{user.name || "ผู้ดูแล"}</p>
        <button onClick={logout} className="text-[14px] text-sub hover:text-red-600">ออกจากระบบ</button>
      </div>
    </div>
  );
}
