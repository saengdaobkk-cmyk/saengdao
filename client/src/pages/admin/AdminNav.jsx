  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="rounded-2xl border border-line bg-white p-2">
        ...
        {!isLoading && items.length === 0 && <p className="px-4 py-3 text-[16px] text-sub">ยังไม่มีเมนู — เพิ่มด้านล่าง</p>}
          ...
          {items.map((n, i) => (
            <li key={n.id} className="flex items-center gap-3 px-2 py-2.5">
              ...
              <div className="min-w-0 flex-1">
                <p className={`truncate text-[17px] font-medium ${n.active ? "text-ink" : "text-sub line-through"}`}>{n.label}</p>
                <p className="truncate text-[15px] text-sub">{n.url}</p>
              </div>
              <Link to={n.url} target="_blank" className="text-[16px] text-sub hover:text-ink">ดู ↗</Link>
              <button onClick={() => toggleActive(n)} className={`text-[16px] ${n.active ? "text-sub hover:text-ink" : "text-accent"}`}>
                {n.active ? "ซ่อน" : "แสดง"}
              </button>
              <button onClick={() => startEdit(n)} className="text-[16px] text-accent">แก้ไข</button>
              <button onClick={() => confirm(`ลบเมนู "${n.label}"?`) && del.mutate(n.id)} className="text-[16px] text-sub hover:text-red-600">ลบ</button>
            </li>
          ))}
        </ul>
      </div>
      ...
      <form onSubmit={submit} className="rounded-2xl border border-line bg-white p-6">
        <p className="mb-3 text-[16px] font-semibold text-ink">{editing ? `แก้ไขเมนู: ${editing.label}` : "เพิ่มเมนูใหม่"}</p>

        <label className="mb-1 block text-[15px] text-sub">เลือกหน้า / collection</label>
        <select
          value={groups.flatMap((g) => g.options).some((o) => o.url === form.url) ? form.url : ""}
          onChange={(e) => pickPage(e.target.value)}
          className="mb-3 w-full rounded-lg border border-line bg-white px-3 py-2 text-[17px] outline-none focus:border-ink/30"
        >
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-[15px] text-sub">ชื่อเมนู (ที่แสดง)</label>
            <input value={form.label} onChange={(e) => setForm((f) => ({ ...f, label: e.target.value }))} placeholder="เช่น นิยาย" className="w-full rounded-lg border border-line bg-white px-3 py-2 text-[17px] outline-none focus:border-ink/30" />
          </div>
          <div>
            <label className="mb-1 block text-[15px] text-sub">ลิงก์ (URL)</label>
            <input value={form.url} onChange={(e) => setForm((f) => ({ ...f, url: e.target.value }))} placeholder="/books?category=fiction หรือ https://..." className="w-full rounded-lg border border-line bg-white px-3 py-2 text-[16px] outline-none focus:border-ink/30" />
          </div>
        </div>

        {error && <p className="mt-2 text-[15px] text-red-600">{error}</p>}
        <div className="mt-4 flex gap-2">
          <button type="submit" disabled={save.isPending} className="rounded-full bg-accent px-5 py-2 text-[16px] font-medium text-white hover:bg-accent/90 disabled:opacity-50">{editing ? "บันทึก" : "เพิ่มเมนู"}</button>
