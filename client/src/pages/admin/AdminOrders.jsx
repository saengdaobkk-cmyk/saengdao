    ...
    <div>
      ...
      <div className="mb-5 flex flex-wrap gap-2">
        {FILTERS.map((f) => (
          ...
          >
            {f.label}
            <span className={`rounded-full px-1.5 text-[12px] ${filter === f.key ? "bg-white/20" : "bg-white text-sub"}`}>
              {counts[f.key]}
            </span>
          </button>
        ))}
      ...
      {/* หัวตาราง (เดสก์ท็อป) */}
      <div className="hidden grid-cols-[1.4fr_1fr_0.8fr_0.9fr_0.7fr_28px] gap-3 border-b border-line px-4 pb-2 text-[13px] text-sub lg:grid">
    ...
    <div className={`bg-white ${open ? "rounded-2xl border border-line shadow-sm" : ""}`}>
      ...
      >
        <div className="min-w-0">
          <p className="truncate text-[15px] font-medium text-ink">
            ...
            <span className="ml-2 text-[13px] font-normal text-sub">{order.user?.name || order.user?.email}</span>
          </p>
          <p className="text-[13px] text-sub lg:hidden">
            {new Date(order.createdAt).toLocaleDateString("th-TH", { dateStyle: "medium" })} · {PAYMENT_LABEL[order.paymentMethod]}
          </p>
        </div>
        <p className="hidden text-[13px] text-sub lg:block">
        ...
        <div className="flex items-center gap-2 justify-self-end lg:contents">
          ...
          <span className="text-[15px] font-semibold text-ink lg:text-right">{formatPrice(order.total)}</span>
          <svg
      ...
      {open && (
        <div className="grid gap-6 border-t border-line px-4 py-5 md:grid-cols-[1.6fr_1fr]">
          ...
          <div className="space-y-5 text-[14px]">
            ...
            <Block title="สถานะคำสั่งซื้อ">
              <select
                ...
                className="w-full max-w-56 rounded-lg border border-line px-3 py-2 text-[14px] outline-none focus:border-ink/30"
              >
                {STATUS.map(([v, l]) => (
          ...
          {/* ขวา: สลิป + อนุมัติ */}
          <div>
            <p className="mb-2 text-[14px] font-medium text-ink">สลิปการชำระเงิน</p>
            {order.slipImage ? (
              ...
              <div className="flex h-32 items-center justify-center rounded-xl bg-mist text-[13px] text-sub">ยังไม่มีสลิป</div>
            )}
            ...
            {order.paymentStatus !== "PAID" ? (
              <>
                <button
                  ...
                  className="mt-3 w-full rounded-full bg-emerald-600 py-2.5 text-[14px] font-medium text-white transition hover:bg-emerald-700 disabled:opacity-50"
                >
                  ✓ อนุมัติการชำระเงิน
                </button>
                {order.paymentStatus === "PENDING_REVIEW" && (
                  <button
                    ...
                    className="mt-2 w-full rounded-full border border-line py-2 text-[13px] text-sub hover:text-red-600"
                  >
                    ปฏิเสธสลิป
                  </button>
                )}
              ...
              <p className="mt-3 rounded-full bg-emerald-50 py-2.5 text-center text-[14px] font-medium text-emerald-700">
                ✓ ชำระเงินแล้ว
              </p>
            )}
    ...
    <Block title="ติดตามพัสดุ (ไปรษณีย์ไทย)">
      <div className="flex flex-wrap items-center gap-2">
        <input
          ...
          className="min-w-52 flex-1 rounded-lg border border-line px-3 py-2 text-[14px] uppercase outline-none focus:border-ink/30"
        />
        <button
          onClick={saveNum}
          disabled={!dirty || update.isPending}
          className="rounded-full bg-ink px-4 py-2 text-[14px] font-medium text-white transition hover:bg-ink/90 disabled:opacity-40"
        >
          บันทึกเลข
        </button>
        <button
          ...
          className="rounded-full border border-line px-4 py-2 text-[14px] font-medium text-ink transition hover:bg-mist disabled:opacity-40"
        >
      ...
      {order.trackingStatus && (
        <div className="mt-3 rounded-xl bg-mist px-3 py-2.5 text-[14px]">
          ...
          {history.length > 1 && (
            <button onClick={() => setShowHistory((s) => !s)} className="mt-1 text-[13px] text-accent">
      ...
      {msg && <p className={`mt-2 text-[13px] ${msg.ok ? "text-emerald-600" : "text-red-600"}`}>{msg.text}</p>}
      {order.trackingNumber && (
        <a
          href={`https://track.thailandpost.co.th/?trackNumber=${order.trackingNumber}`}
          target="_blank"
          rel="noreferrer"
          className="mt-2 inline-block text-[13px] text-sub hover:text-ink"
        >
          เปิดหน้าไปรษณีย์ไทย ↗
        </a>
      )}
  ...
  return (
    <div className="mt-4 border-t border-line pt-3">
      <div className="flex items-center justify-between">
        <span className="text-[13px] font-semibold uppercase tracking-wide text-sub">ZORT</span>
        {order.zortOrderId ? (
          <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-[12px] font-medium text-emerald-700">
          ...
          <button
            ...
            className="rounded-full border border-line px-3 py-1.5 text-[13px] font-medium text-ink transition hover:bg-mist disabled:opacity-50"
          >
...
function OrderInfo({ order }) {
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          <div className="mb-1.5 flex items-center gap-2">
            <p className="text-[13px] font-semibold uppercase tracking-wide text-sub">ที่อยู่จัดส่ง</p>
            <button onClick={startEdit} className="text-[13px] text-accent">แก้ไข</button>
          </div>
  ...
  return (
    <div className="rounded-xl border border-line bg-mist/40 p-4">
      <p className="mb-3 text-[13px] font-semibold uppercase tracking-wide text-sub">แก้ไขข้อมูลจัดส่ง</p>
      ...
      <div className="mt-4 flex gap-2">
        <button onClick={save} disabled={update.isPending} className="rounded-full bg-accent px-5 py-2 text-[14px] font-medium text-white transition hover:bg-accent/90 disabled:opacity-50">
          {update.isPending ? "กำลังบันทึก..." : "บันทึก"}
        </button>
        <button onClick={() => setEditing(false)} className="rounded-full border border-line px-4 py-2 text-[14px] text-ink hover:bg-white">ยกเลิก</button>
      </div>
    </div>
  );
}
...
function EditField({ label, value, onChange, className = "" }) {
  return (
    <label className={`block ${className}`}>
      <span className="mb-1 block text-[12px] font-medium text-sub">{label}</span>
      <input value={value} onChange={onChange} className="w-full rounded-lg border border-line bg-white px-3 py-2 text-[14px] text-ink outline-none focus:border-ink/30" />
    </label>
  );
}

function Badge({ cls, children }) {
  return <span className={`inline-block rounded-full px-2.5 py-1 text-[12px] font-medium ${cls}`}>{children}</span>;
}

function Block({ title, children }) {
  return (
    <div>
      <p className="mb-1.5 text-[13px] font-semibold uppercase tracking-wide text-sub">{title}</p>
