  ...
  return (
    <div>
      ...
      <div className="mb-5 flex flex-wrap items-center gap-2">
        <h2 className="mr-2 text-[18px] font-semibold text-ink">หนังสือทั้งหมด ({books?.length || 0})</h2>
        ...
        <select value={cat} onChange={(e) => setCat(e.target.value)} className="rounded-lg border border-line px-3 py-2 text-[16px] outline-none focus:border-ink/30">
          <option value="">ทุกหมวด</option>
          {categories?.map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
        </select>
        <select value={status} onChange={(e) => setStatus(e.target.value)} className="rounded-lg border border-line px-3 py-2 text-[16px] outline-none focus:border-ink/30">
        ...
        <div className="ml-auto flex gap-2">
          <button onClick={() => setImporting(true)} className="rounded-full border border-line px-5 py-2.5 text-[17px] font-medium text-ink transition hover:bg-mist">
            ⬆ นำเข้า CSV/Excel
          </button>
          <button onClick={() => setEditing(EMPTY)} className="rounded-full bg-accent px-5 py-2.5 text-[17px] font-medium text-white transition hover:bg-accent/90">
            + เพิ่มหนังสือ
          </button>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="overflow-x-auto rounded-2xl border border-line bg-white">
          <table className="w-full text-left text-[17px]">
            <thead className="border-b border-line bg-mist/50 text-[15px] text-sub">
              <tr>
            ...
            <tbody className="divide-y divide-line">
              {filtered.map((b) => (
                <tr key={b.id} className={`hover:bg-mist/30 ${b.active === false ? "bg-mist/20" : ""}`}>
                  <td className="px-4 py-3 text-[15px] text-sub">{b.isbn || "—"}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className={`flex h-12 w-9 shrink-0 items-center justify-center overflow-hidden rounded bg-mist ${b.active === false ? "opacity-40" : ""}`}>
                        {b.coverImage ? <img src={b.coverImage} alt="" className="h-full w-full object-cover" /> : <span className="text-[13px] opacity-30">𝐀</span>}
                      </div>
                      <div>
                  ...
                  <td className="px-4 py-3">
                    {b.discountPrice != null ? (
                      <span><span className="text-ink">{formatPrice(b.discountPrice)}</span> <span className="text-[14px] text-sub line-through">{formatPrice(b.price)}</span></span>
                  ...
                  {(() => {
                    ...
                    return (
                      <td className={`px-4 py-3 ${s <= 5 ? "text-amber-600" : "text-ink"}`}>
                        {s}{hasVar && <span className="ml-1 text-[14px] text-sub">({b.variants.length} แบบ)</span>}
                      </td>
                    );
                  })()}
                  <td className="whitespace-nowrap px-4 py-3 text-right">
                    ...
                    <button onClick={() => setEditing(b)} className="ml-2 rounded-lg border border-line px-3 py-1 text-[15px] text-ink hover:bg-mist">แก้ไข</button>
                    <button onClick={() => confirm(`ลบ "${b.title}"?`) && del.mutate(b.id, { onError: (e) => alert(e.response?.data?.error || "ลบไม่สำเร็จ") })}
                      className="ml-2 rounded-lg border border-line px-3 py-1 text-[15px] text-sub hover:text-red-600">ลบ</button>
                  </td>
                </tr>
              ))}
            </tbody>
...
function Tag({ children, color }) {
  ...
  return <span className={`rounded px-1.5 py-0.5 text-[13px] ${c}`}>{children}</span>;
}
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
        ...
        <div className="space-y-6">
          <Card>
            ...
            <F label="รายละเอียด"><textarea value={form.description} onChange={set("description")} rows={5} className="w-full resize-y rounded-xl border border-line px-4 py-2.5 text-[17px] outline-none focus:border-ink/30" /></F>
          </Card>
          ...
          <Card title="รหัสสินค้า & SEO">
            ...
            <F label="Meta Description" hint="เว้นว่าง = ใช้รายละเอียด · ≤ 160 ตัวอักษร"><textarea value={form.metaDescription} onChange={set("metaDescription")} rows={2} className="w-full resize-y rounded-xl border border-line px-4 py-2.5 text-[17px] outline-none focus:border-ink/30" /></F>
            ...
            <F label="ไฟล์ตัวอย่าง (PDF)" hint="อ่านตัวอย่างแบบ flip book · ≤ 25MB">
              {form.previewPdf && <a href={form.previewPdf} target="_blank" rel="noreferrer" className="mb-2 block text-[15px] text-accent">เปิดดู PDF ปัจจุบัน</a>}
              <label className="flex cursor-pointer items-center justify-center rounded-xl border border-dashed border-line py-2 text-[16px] text-sub hover:text-ink">
              ...
              <div className="space-y-3">
                {form.variants.map((v, i) => (
                  <div key={i} className="space-y-3 rounded-xl border border-line p-3">
                    <div className="grid grid-cols-[1fr_130px_90px_90px_70px_28px] gap-2 text-[14px] text-sub">
                    ...
                    <div className="flex flex-wrap gap-4 border-t border-line pt-3">
                      ...
                      <p className="max-w-[180px] self-center text-[14px] text-sub">เว้นว่าง = ใช้ปกหลักของเล่ม · ใส่เฉพาะแบบที่ปกต่างกัน</p>
                    </div>
                  </div>
                ))}
              </div>
            )}
            <button type="button" onClick={addVariant} className="mt-3 rounded-xl border border-dashed border-line px-4 py-2 text-[16px] text-ink hover:bg-mist">+ เพิ่มตัวเลือก</button>
          </Card>
          ...
          <Card title="รูปภาพเพิ่มเติม" subtitle="ปกหน้าใช้ช่อง “รูปปก” ทางขวา · ตรงนี้เพิ่ม ปกหลัง + รูปแกลเลอรี">
            <F label="ปกหลัง (กดพลิกได้)">
              ...
              {form.backCoverImage && <button type="button" onClick={() => setForm((f) => ({ ...f, backCoverImage: "" }))} className="mt-1 text-[15px] text-sub hover:text-red-600">ลบปกหลัง</button>}
            </F>
                ...
                {form.galleryImages.map((url, i) => (
                  <div key={i} className="relative">
                    ...
                    <button type="button" onClick={() => setForm((f) => ({ ...f, galleryImages: f.galleryImages.filter((_, idx) => idx !== i) }))}
                      className="absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-ink text-[13px] text-white">✕</button>
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-6 lg:sticky lg:top-20 lg:self-start">
          <Card>
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[16px] font-medium text-ink">สถานะการขาย</p>
                <p className="text-[15px] text-sub">{form.active !== false ? "เปิดขาย — แสดงในหน้าร้าน" : "ปิดอยู่ — ซ่อนจากหน้าร้าน"}</p>
              </div>
          ...
          <Card>
            <div className="grid grid-cols-2 gap-3">
              ...
              <label className="flex items-end gap-2 pb-2 text-[16px] text-ink">
          ...
          <button type="submit" disabled={save.isPending} className="w-full rounded-full bg-accent py-3 text-[18px] font-medium text-white transition hover:bg-accent/90 disabled:opacity-50">
            {save.isPending ? "กำลังบันทึก..." : "บันทึกการแก้ไข"}
          </button>
          <button type="button" onClick={onClose} className="w-full rounded-full border border-line py-3 text-[18px] text-ink hover:bg-mist">ยกเลิก</button>
        </div>
      </div>
    </form>
  );
...
const Card = ({ title, subtitle, children }) => (
  <div className="space-y-4 rounded-2xl border border-line bg-white p-6">
    {title && <div><p className="text-[18px] font-semibold text-ink">{title}</p>{subtitle && <p className="mt-0.5 text-[15px] text-sub">{subtitle}</p>}</div>}
    {children}
  </div>
);
const F = ({ label, hint, children }) => (
  <label className="block">
    ...
    {children}
    {hint && <span className="mt-1 block text-[14px] text-sub">{hint}</span>}
  </label>
);
const Inp = (props) => <input {...props} className="w-full rounded-xl border border-line px-4 py-2.5 text-[17px] text-ink outline-none focus:border-ink/30" />;
const Select = (props) => <select {...props} className="w-full rounded-xl border border-line px-4 py-2.5 text-[17px] text-ink outline-none focus:border-ink/30" />;
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex h-16 w-11 shrink-0 items-center justify-center overflow-hidden rounded bg-mist">
        {url ? <img src={url} alt="" className="h-full w-full object-cover" /> : <span className="text-[12px] text-sub">{label}</span>}
      </div>
