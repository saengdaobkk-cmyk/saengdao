...
export default function AdminSettings() {
  ...
  return (
    <div className="space-y-12">
      {/* การแสดงผล */}
      <section>
        <h2 className="mb-4 text-[16px] font-semibold text-ink">การแสดงผล</h2>
...
function ContactSettings({ settings, save }) {
  ...
  return (
    <section>
      <h2 className="mb-1 text-[16px] font-semibold text-ink">ข้อมูลติดต่อ</h2>
      <p className="mb-4 text-[13px] text-sub">แสดงบนหน้า "ติดต่อเรา" และ footer</p>
      <form onSubmit={submit} className="space-y-6 rounded-2xl border border-line bg-white p-6">
        <div>
          <p className="mb-3 text-[14px] font-semibold text-ink">ติดต่อ</p>
        ...
        <div>
          <p className="mb-3 text-[14px] font-semibold text-ink">โซเชียล (ลิงก์เพจ)</p>
          <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="flex items-center gap-4">
          ...
          {saved && <span className="text-[14px] text-emerald-600">บันทึกแล้ว</span>}
        </div>
      </form>
    </section>
  );
...
function PaymentSettings({ settings, save }) {
  ...
  return (
    <section>
      <h2 className="mb-1 text-[16px] font-semibold text-ink">การชำระเงิน</h2>
      <p className="mb-4 text-[13px] text-sub">แสดงให้ลูกค้าเห็นตอนชำระเงิน (QR สร้างจากพร้อมเพย์อัตโนมัติ)</p>

      <form onSubmit={submit} className="space-y-6 rounded-2xl border border-line bg-white p-6">
        <div>
          <p className="mb-3 text-[14px] font-semibold text-ink">พร้อมเพย์</p>
          <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          <p className="mb-3 text-[14px] font-semibold text-ink">บัญชีธนาคาร</p>
          <div className="grid gap-4 sm:grid-cols-2">
  ...
  return (
    <div className="flex items-center justify-between gap-6 px-5 py-4">
      <div>
        ...
        <p className="mt-0.5 text-[13px] leading-relaxed text-sub">{desc}</p>
      </div>
      <button
        role="switch"
...
function Input({ label, value, onChange, placeholder, className = "" }) {
  return (
    <label className={`block ${className}`}>
      ...
      <input value={value} onChange={onChange} placeholder={placeholder} className="w-full rounded-xl border border-line bg-white px-4 py-2.5 text-[15px] text-ink outline-none transition placeholder:text-sub/50 focus:border-ink/30" />
    </label>
  );
}
