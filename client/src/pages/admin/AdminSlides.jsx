  ...
  return (
    <div>
      <div className="mb-5 flex items-center justify-between">
        <p className="text-[17px] text-sub">{slides?.length || 0} สไลด์ · เรียงตามลำดับ order</p>
        <button
          onClick={() => setEditing({ ...EMPTY, order: (slides?.length || 0) + 1 })}
          className="rounded-full bg-ink px-5 py-2.5 text-[17px] font-medium text-white transition hover:bg-ink/90"
        >
          + เพิ่มสไลด์
        </button>
      </div>
        ...
        <div className="space-y-3">
          {slides.map((s) => (
            <div key={s.id} className="flex items-center gap-4 rounded-2xl border border-line bg-white p-3">
              ...
              >
                <span className={`px-1 text-[13px] font-semibold leading-tight ${s.dark ? "text-white" : "text-ink"}`}>
                  {s.title.split("\n")[0]}
                </span>
              </div>

              <div className="flex-1">
                <p className="text-[17px] font-medium text-ink">{s.title.replace(/\n/g, " ")}</p>
                <p className="text-[15px] text-sub">
                  #{s.order} · {s.eyebrow || "ไม่มี eyebrow"} {s.image ? "· มีรูป" : "· พื้นสี"}
                </p>
              </div>

              <span className={`rounded-full px-2.5 py-1 text-[14px] font-medium ${s.active ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>
                {s.active ? "แสดง" : "ซ่อน"}
              </span>
              <button onClick={() => setEditing(s)} className="text-[16px] text-accent">แก้ไข</button>
              <button
                onClick={() => confirm("ลบสไลด์นี้?") && del.mutate(s.id)}
                className="text-[16px] text-sub hover:text-red-600"
              >
                ลบ
              </button>
            </div>
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-ink/40 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6" onClick={(e) => e.stopPropagation()}>
        <h3 className="mb-5 text-[20px] font-semibold text-ink">{slide.id ? "แก้ไขสไลด์" : "เพิ่มสไลด์"}</h3>
        ...
        >
          ...
          <div className={`relative max-w-[70%] ${form.dark ? "text-white" : "text-ink"}`} style={form.textColor ? { color: form.textColor } : undefined}>
            {form.eyebrow && <p className="text-[13px] opacity-80">{form.eyebrow}</p>}
            <p className="whitespace-pre-line text-lg font-semibold leading-tight">{form.title || "หัวข้อสไลด์"}</p>
            {form.ctaText && (
              <span
                className="mt-2 inline-block rounded-full bg-accent px-3 py-1 text-[14px] text-white"
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <F label="หัวข้อใหญ่ * (Enter ขึ้นบรรทัดใหม่)">
            <textarea value={form.title} onChange={set("title")} rows={2} className="w-full resize-none rounded-xl border border-line px-4 py-2.5 text-[17px] outline-none focus:border-ink/30" />
          </F>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <F label="รูปพื้นหลัง — แนะนำ 1920×1080 (16:9), < 500KB">
              <div className="flex gap-2">
                <label className="flex flex-1 cursor-pointer items-center justify-center rounded-xl border border-dashed border-line py-2 text-[16px] text-sub hover:text-ink">
                  {uploading ? "กำลังอัปโหลด..." : form.image ? "เปลี่ยนรูป" : "อัปโหลดรูป"}
                  <input type="file" accept="image/*" onChange={onFile} className="hidden" />
                </label>
                {form.image && (
                  <button type="button" onClick={() => setForm((f) => ({ ...f, image: "" }))} className="rounded-xl border border-line px-3 text-[16px] text-sub hover:text-red-600">
                    เอาออก
                  </button>
                )}
              </div>
          ...
          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={save.isPending} className="flex-1 rounded-full bg-accent py-2.5 text-[17px] font-medium text-white transition hover:bg-accent/90 disabled:opacity-50">
              {save.isPending ? "กำลังบันทึก..." : "บันทึก"}
            </button>
            <button type="button" onClick={onClose} className="rounded-full border border-line px-6 py-2.5 text-[17px] text-ink hover:bg-mist">ยกเลิก</button>
          </div>
        </form>
      </div>
    </div>
...
function ColorField({ label, value, fallback, onChange }) {
  return (
    <label className="block">
      <span className="mb-1.5 flex items-center justify-between text-[15px] font-medium text-sub">
        {label}
        {value && (
          <button type="button" onClick={() => onChange("")} className="text-[14px] text-accent">
            รีเซ็ต
          </button>
        )}
      </span>
      <div className="flex items-center gap-2">
        ...
        <input
          ...
          className="w-full rounded-xl border border-line px-3 py-2 text-[15px] text-ink outline-none focus:border-ink/30"
        />
      </div>
    </label>
  );
...
const Inp = (props) => (
  <input {...props} className="w-full rounded-xl border border-line px-4 py-2.5 text-[17px] text-ink outline-none focus:border-ink/30" />
);
