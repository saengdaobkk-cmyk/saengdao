  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
              ...
              return (
                <tr key={u.id} className="hover:bg-mist/30">
                  <td className="px-5 py-3">
                    <p className="font-medium text-ink">{u.name || "—"} {self && <span className="text-[11px] text-sub">(คุณ)</span>}</p>
                    <p className="text-[12px] text-sub">{u.email}</p>
                  </td>
                  <td className="px-5 py-3">
                  ...
                  <td className="whitespace-nowrap px-5 py-3">
                    <div className="flex justify-end gap-4">
                      ...
                      <button
                        ...
                        className="text-[13px] text-sub hover:text-red-600 disabled:opacity-30">ลบ</button>
                    </div>
                  </td>
                </tr>
              );
      ...
      <form onSubmit={submit} className="h-fit space-y-3 rounded-2xl border border-line bg-white p-5 lg:sticky lg:top-20">
        <p className="text-[13px] font-semibold text-ink">{editing ? `แก้ไข: ${editing.email}` : "เพิ่มผู้ใช้ใหม่"}</p>

        <label className="block">
          <span className="mb-1 block text-[12px] text-sub">อีเมล</span>
          <input type="email" value={form.email} disabled={!!editing} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))}
            placeholder="staff@saengdao.com" className="w-full rounded-lg border border-line bg-white px-3 py-2 text-[14px] outline-none focus:border-ink/30 disabled:bg-mist disabled:text-sub" />
        </label>
        <label className="block">
          <span className="mb-1 block text-[12px] text-sub">ชื่อ</span>
          <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
            placeholder="ชื่อ-นามสกุล" className="w-full rounded-lg border border-line bg-white px-3 py-2 text-[14px] outline-none focus:border-ink/30" />
        </label>
        <label className="block">
          <span className="mb-1 block text-[12px] text-sub">{editing ? "รหัสผ่านใหม่ (เว้นว่าง = คงเดิม)" : "รหัสผ่าน"}</span>
          <input type="password" value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))}
            placeholder="อย่างน้อย 6 ตัวอักษร" className="w-full rounded-lg border border-line bg-white px-3 py-2 text-[14px] outline-none focus:border-ink/30" />
        </label>
        <label className="block">
          <span className="mb-1 block text-[12px] text-sub">สิทธิ์</span>
          <select value={form.role} disabled={editing?.id === me?.id} onChange={(e) => setForm((f) => ({ ...f, role: e.target.value }))}
            className="w-full rounded-lg border border-line bg-white px-3 py-2 text-[14px] outline-none focus:border-ink/30 disabled:bg-mist">
            <option value="STAFF">พนักงาน — สินค้า/คำสั่งซื้อ/Collection</option>
            <option value="ADMIN">แอดมินเต็ม — ทุกเมนู</option>
          </select>
          <span className="mt-1 block text-[11px] text-sub/70">{ROLE_META[form.role]?.desc}</span>
        </label>

        {error && <p className="text-[12px] text-red-600">{error}</p>}
        <div className="flex gap-2 pt-1">
          <button type="submit" disabled={save.isPending} className="rounded-full bg-accent px-5 py-2 text-[13px] font-medium text-white hover:bg-accent/90 disabled:opacity-50">{editing ? "บันทึก" : "เพิ่มผู้ใช้"}</button>
